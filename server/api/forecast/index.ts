...
export default defineEventHandler(
  async (): Promise<UnguardCurrentCondition | WeatherApiError> => {
    ...
    try {
      ...
      if (currentCondition) {
        console.log(
          'Successfully adapted API response to UnguardCurrentCondition',
        )
        console.log(currentCondition.wind.direction)
        return currentCondition
      }
      else {
        console.error(
          'Failed to adapt API response to UnguardCurrentCondition',
        )
        return {
          error: { code: 422, message: 'Failed to process weather data' },
        }
      }
    }
    catch (error) {
