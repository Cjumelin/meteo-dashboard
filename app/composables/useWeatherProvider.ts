...
export const useWeatherProvider = () => {
  ...
  const weatherApiCall = async (): Promise<
    UnguardCurrentCondition | WeatherApiError
  > => {
    return await $fetch('/api/forecast')
  }

  const retrieveCurrentCondition
    = async (): Promise<CurrentCondition | null> => {
      loading.value = true
      error.value = null
      ...
      try {
        ...
        console.log(
          'Successfully retrieved current condition:',
          domainCondition,
        )
